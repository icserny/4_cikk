/**********************************************************************
 *  Launchpad demóprogram: ADC_pwm1
 *
 * A P1.6 kimenetre kötött zöld LED fényerejét impulzus-szélesség
 * modulációval (PWM) váltaoztatjuk, melyhez a vezérlõ jelet a P1.5
 ...
 * PWM kitöltés frissítése a CCIFG0 megszakításban történik, ahol 
 * egyúttal újabb konverziót indítunk a következõ frissításhez.
 *
 * Hardver feltételek:
...
__interrupt void TIMERA0_ISR (void)    // A jelzõbit automatikusan törlõdik
{
    TACCR1 = ADC10MEM;                 // PWM kitöltés = az elõzõ ADC mérés 
