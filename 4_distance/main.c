...
#define ECHO      BIT2            //Echo jel a HC-SR04-bõl (CCI1A bemenet)
#define SW2       BIT3            //Nyomógomb a P1.3 lábon
#define TRIGGER   BIT5            //Trigger kimenet HC-SR04-hez
...
//-- HC-SR04 vezérlés konfigurálása ---------------------------
  ...
  P1SEL |= ECHO;                  //TA0CCI1A engedélyezése
//  P1SEL2 &= ECHO;
  while(1) {
    delta = 0;                    //Nullázzuk a munkaváltozót
//-- Timer0 és TA0CCR1 konfigurálása ---------------------------
    TA0CTL = TASSEL_2 |           //Timer_A forrása SMCLK legyen
                 ID_0 |           //Bemeneti osztó = 1:1
                 MC_2 |           //Folyamatos számlálás 
                 TACLR;           //A számlálót töröljük    
   TA0CCR1 = 0;                   //CCR1 adatregiszterét töröljük
   TA0CCTL1 =    CM_1 |           //Felfutó élre érzékeny 
               CCIS_0 |           //TA0CCI1A bemenet kiválasztása
...
//-- Trigger impulzussal elindítjuk a mérést           ____________
    ...
    __low_power_mode_0();              //Várunk a Capture ciklus lezajlására
...
__interrupt void ta1_isr(void) {
  TA0CCTL1 &= ~CCIFG;                  //A megszakítási jelzõbit törlése
  if(TA0CCTL1&CCI) {
    last_capture = TA0CCR1;            //A felfutó élhez tartozó idõ rögzítése
     TA0CCTL1 ^= CM_3;                 //Lefutó élre érzékenyítünk
  } 
  else {
    TA0CCTL1 &= ~CM_3;                 //Capture ciklus vége
